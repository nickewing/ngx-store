...
import { merge, set as lset, get as lget } from "lodash";
...
export class Resource<T> {
  ...
  public save(value: T): this {
    if (this.pathString) {
      value = lset(<any>this.fullValue, this.pathString, this.considerDefault(value));
    }
    this.service.utility.set(this.key, this.considerDefault(value), {
      prefix: this._prefix
    });
    return this;
  }
  ...
  public update(value: T): this {
    return this.save(merge(this.readValue(), value));
  }
  ...
  protected get fullValue(): T {
    return this.considerDefault(
      this.service.utility.get(this.key, { prefix: this._prefix })
    );
  }
  ...
  protected readValue(): T {
    ...
    if (this.pathString) {
      return lget(value, this.pathString);
    }
    return value;
  }
}
